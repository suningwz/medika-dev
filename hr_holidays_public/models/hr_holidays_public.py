...
class HrHolidaysPublic(models.Model):
    ...
    year = fields.Integer(string="Calendar Year", required=True, default=str(date.today().year))
    line_ids = fields.One2many("hr.holidays.public.line", "year_id", "Holiday Dates")
    country_id = fields.Many2one("res.country", "Country", default=lambda self: self.env.company.country_id)
    ...
    year_str = fields.Char(string='Calendar Year', default=str(date.today().year), size=4, required=True, )

    @api.onchange('year_str')
    def _onchange_year_str(self):
        for r in self:
            if r.year_str:
                if not r.year_str.isdigit():
                    r.year_str = ''
                    raise ValidationError('Please input number only')
                if len(r.year_str) != 4:
                    r.year_str = ''
                    raise ValidationError('Year not found')
                else:
                    r.year = int(r.year_str)
