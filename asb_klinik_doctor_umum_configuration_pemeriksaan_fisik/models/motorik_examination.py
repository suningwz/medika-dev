...
class MotorikExamination(models.Model):
    _name           = 'motorik.examination'
    _description    = 'Motorik Examination'
...
class MasterRegistration(models.Model):
    ...
    motorik_examination_line = fields.One2many( 'motorik.examination', 'registration_id', 
                                                string = 'Motorik Examination', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        motorik_examination_line = []
        motorik_examination_rec = self.env['pemeriksaan.fisik.master'].search([('tipe_pemeriksaan_fisik', '=', 'motorik')])
        for rec in motorik_examination_rec:
            line = (0, 0, {
                'pemeriksaan_fisik_master_id': rec.id
            })
            motorik_examination_line.append(line)
        res.update({
            'motorik_examination_line': motorik_examination_line
        })
        return res
...
class PemeriksaanFisikMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.tipe_pemeriksaan_fisik == 'motorik':
                rec.write({'motorik_examination_line': x})
        return res