...
class LimfatikSistem(models.Model):
    _name           = 'limfatik.sistem'
    _description    = 'Limfatik Sistem'
...
class MasterRegistration(models.Model):
    ...
    limfatik_sistem_line = fields.One2many( 'limfatik.sistem', 'registration_id', 
                                            string = 'Limfatik Sistem', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        limfatik_sistem_line = []
        limfatik_sistem_rec = self.env['pemeriksaan.fisik.master'].search([('tipe_pemeriksaan_fisik', '=', 'limfatik')])
        for rec in limfatik_sistem_rec:
            line = (0, 0, {
                'pemeriksaan_fisik_master_id': rec.id
            })
            limfatik_sistem_line.append(line)
        res.update({
            'limfatik_sistem_line': limfatik_sistem_line
        })
        return res
...
class PemeriksaanFisikMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.tipe_pemeriksaan_fisik == 'limfatik':
                rec.write({'limfatik_sistem_line': x})
        return res
