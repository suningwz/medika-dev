...
class MasterRegistration(models.Model):
    ...
    tht_ent_line    = fields.One2many('tht.ent', 'registration_id', string = 'tht ent', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        tht_ent_line = []
        tht_ent_rec = self.env['pemeriksaan.fisik.master'].search([('tipe_pemeriksaan_fisik', '=', 'tht')])
        for rec in tht_ent_rec:
            line = (0, 0, {
                'pemeriksaan_fisik_master_id': rec.id
            })
            tht_ent_line.append(line)
        res.update({
            'tht_ent_line': tht_ent_line
        })
        return res
...
class PemeriksaanFisikMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.tipe_pemeriksaan_fisik == 'tht':
                rec.write({'tht_ent_line': x})
        return res