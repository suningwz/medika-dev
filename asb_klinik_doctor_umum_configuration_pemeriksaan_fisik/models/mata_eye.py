...
class MasterRegistration(models.Model):
    ...
    mata_eye_line   = fields.One2many('mata.eye', 'registration_id', string = 'Mata Eye', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        mata_eye_line = []
        mata_eye_rec = self.env['pemeriksaan.fisik.master'].search([('tipe_pemeriksaan_fisik', '=', 'mata')])
        for rec in mata_eye_rec:
            line = (0, 0, {
                'pemeriksaan_fisik_master_id': rec.id
            })
            mata_eye_line.append(line)
        res.update({
            'mata_eye_line': mata_eye_line
        })
        return res
...
class PemeriksaanFisikMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.tipe_pemeriksaan_fisik == 'mata':
                rec.write({'mata_eye_line': x})
        return res