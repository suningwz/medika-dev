...
class MasterRegistration(models.Model):
    ...
    kulit_skin_line     = fields.One2many( 'kulit.skin', 'registration_id', 
                                           string = 'Kulit Skin', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        kulit_skin_line = []
        kulit_skin_rec = self.env['pemeriksaan.fisik.master'].search([('tipe_pemeriksaan_fisik', '=', 'kulit')])
        for rec in kulit_skin_rec:
            line = (0, 0, {
                'pemeriksaan_fisik_master_id': rec.id
            })
            kulit_skin_line.append(line)
        res.update({
            'kulit_skin_line': kulit_skin_line
        })
        return res
...
class PemeriksaanFisikMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.tipe_pemeriksaan_fisik == 'kulit':
                rec.write({'kulit_skin_line': x})
        return res
