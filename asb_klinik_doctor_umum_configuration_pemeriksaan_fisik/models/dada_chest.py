...
class MasterRegistration(models.Model):
    ...
    dada_chest_line = fields.One2many( 'dada.chest', 'registration_id', 
                                       string = 'Dada Chest', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        dada_chest_line = []
        dada_chest_rec = self.env['pemeriksaan.fisik.master'].search([('tipe_pemeriksaan_fisik', '=', 'dada')])
        for rec in dada_chest_rec:
            line = (0, 0, {
                'pemeriksaan_fisik_master_id': rec.id
            })
            dada_chest_line.append(line)
        res.update({
            'dada_chest_line': dada_chest_line
        })
        return res
...
class PemeriksaanFisikMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.tipe_pemeriksaan_fisik == 'dada':
                rec.write({'dada_chest_line': x})
        return res