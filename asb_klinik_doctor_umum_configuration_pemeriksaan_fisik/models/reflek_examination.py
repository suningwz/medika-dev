...
class ReflekExamination(models.Model):
    _name           = 'reflek.examination'
    _description    = 'Reflek Examination'
...
class MasterRegistration(models.Model):
    ...
    reflek_examination_line = fields.One2many( 'reflek.examination', 'registration_id', 
                                               string = 'Reflek Examination', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        reflek_examination_line = []
        reflek_examination_rec = self.env['pemeriksaan.fisik.master'].search([('tipe_pemeriksaan_fisik', '=', 'reflek')])
        for rec in reflek_examination_rec:
            line = (0, 0, {
                'pemeriksaan_fisik_master_id': rec.id
            })
            reflek_examination_line.append(line)
        res.update({
            'reflek_examination_line': reflek_examination_line
        })
        return res
...
class PemeriksaanFisikMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.tipe_pemeriksaan_fisik == 'reflek':
                rec.write({'reflek_examination_line': x})
        return res
