...
class MasterRegistration(models.Model):
    ...
    leher_neck_line = fields.One2many( 'leher.neck', 'registration_id', 
                                       string = 'Leher Neck', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        leher_neck_line = []
        leher_neck_rec = self.env['pemeriksaan.fisik.master'].search([('tipe_pemeriksaan_fisik', '=', 'leher')])
        for rec in leher_neck_rec:
            line = (0, 0, {
                'pemeriksaan_fisik_master_id': rec.id
            })
            leher_neck_line.append(line)
        res.update({
            'leher_neck_line': leher_neck_line
        })
        return res
...
class PemeriksaanFisikMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.tipe_pemeriksaan_fisik == 'leher':
                rec.write({'leher_neck_line': x})
        return res