...
class MemberHistory(models.Model):
    ...
    import_remarks = fields.Char(string='Import Remarks')
    ...
    def open_member_history(self):
        view_id = self.env.ref('asb_membership_member.member_history_view_form').id
        return {
            'view_mode': 'form',
            'views': [(view_id, 'form')],
            'res_model': 'member.history',
            'view_id': view_id,
