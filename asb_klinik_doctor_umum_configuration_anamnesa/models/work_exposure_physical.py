...
class WorkExposurePhysical(models.Model):
    _name               = 'work.exposure.physical'
    _description        = 'Work Exposure Physical'
...
class MasterRegistration(models.Model):
    ...
    work_exposure_physical_line = fields.One2many( 'work.exposure.physical', 'registration_id', 
                                                   string = 'Work Exposure Physical', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        work_exposure_physical_line = []
        work_exposure_physical_rec = self.env['anamnesa.master'].search([('anamnesa_type', '=', 'exposure_physical')])
        for rec in work_exposure_physical_rec:
            line = (0, 0, {
                'anamnesa_master_id': rec.id
            })
            work_exposure_physical_line.append(line)
        res.update({
            'work_exposure_physical_line': work_exposure_physical_line
        })
        return res
...
class AnamnesaMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.anamnesa_type == 'exposure_physical':
                rec.write({'work_exposure_physical_line': x})
        return res
