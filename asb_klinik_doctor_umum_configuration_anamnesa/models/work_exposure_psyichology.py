...
class WorkExposurePsyichology(models.Model):
    _name           = 'work.exposure.psyichology'
    _description    = 'Work Exposure Psyichology'
...
class MasterRegistration(models.Model):
    ...
    work_exposure_psyichology_line = fields.One2many( 'work.exposure.psyichology', 'registration_id', 
                                                      string = 'Work Exposure Psyichology', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        work_exposure_psyichology_line = []
        work_exposure_psyichology_rec = self.env['anamnesa.master'].search([('anamnesa_type', '=', 'exposure_psyichology')])
        for rec in work_exposure_psyichology_rec:
            line = (0, 0, {
                'anamnesa_master_id': rec.id
            })
            work_exposure_psyichology_line.append(line)
        res.update({
            'work_exposure_psyichology_line': work_exposure_psyichology_line
        })
        return res
...
class AnamnesaMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.anamnesa_type == 'exposure_psyichology':
                rec.write({'work_exposure_psyichology_line': x})
        return res
