...
class WorkExposureChemical(models.Model):
    _name           = 'work.exposure.chemical'
    _description    = 'Work Exposure Chemical'
...
class MasterRegistration(models.Model):
    ...
    work_exposure_chemical_line = fields.One2many( 'work.exposure.chemical', 'registration_id', 
                                                   string = 'Work Exposure chemical', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        work_exposure_chemical_line = []
        work_exposure_chemical_rec = self.env['anamnesa.master'].search([('anamnesa_type', '=', 'exposure_chemical')])
        for rec in work_exposure_chemical_rec:
            line = (0, 0, {
                'anamnesa_master_id': rec.id
            })
            work_exposure_chemical_line.append(line)
        res.update({
            'work_exposure_chemical_line': work_exposure_chemical_line
        })
        return res
...
class AnamnesaMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.anamnesa_type == 'exposure_chemical':
                rec.write({'work_exposure_chemical_line': x})
        return res
