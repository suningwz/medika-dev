...
class WorkExposureErgonomics(models.Model):
    _name           = 'work.exposure.ergonomics'
    _description    = 'Work Exposure Ergonomics'
...
class MasterRegistration(models.Model):
    ...
    work_exposure_ergonomics_line = fields.One2many( 'work.exposure.ergonomics', 'registration_id', 
                                                     string = 'Work Exposure ergonomics', tracking = True)

    @api.model
    def default_get(self, fields):
        res = super(MasterRegistration, self).default_get(fields)
        work_exposure_ergonomics_line = []
        work_exposure_ergonomics_rec = self.env['anamnesa.master'].search([('anamnesa_type', '=', 'exposure_ergonomics')])
        for rec in work_exposure_ergonomics_rec:
            line = (0, 0, {
                'anamnesa_master_id': rec.id
            })
            work_exposure_ergonomics_line.append(line)
        res.update({
            'work_exposure_ergonomics_line': work_exposure_ergonomics_line
        })
        return res
...
class AnamnesaMaster(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in registration_rec:
            if res.anamnesa_type == 'exposure_ergonomics':
                rec.write({'work_exposure_ergonomics_line': x})
        return res
